...
sprayFile := Athlete360.files_spray.WSOCgymawarefile;

// get the layout (processed layout)
stgLayout := Athlete360.Layouts.WSOCgymaware_stg;

// do all preprocessing actions and get the cleaned data from spray
stgLayout extractdata (Athlete360.Layouts.WSOCgymaware L):= transform
...
finalStageData := DEDUP(
        SORT(
            cleanedSprayFile + Athlete360.files_stg.WSOCgymaware_stgfile,
            NAME, DATE, -wuid),
        NAME, DATE
    );
...
completestgdata := join(dedup(sort(Athlete360.files_stg.WSOCgymaware_stgfile, name), name),
...
EXPORT build_WSOCgymaware := Athlete360.util.fn_promote_ds(Athlete360.util.constants.stg_prefix,  Athlete360.util.constants.WSOCgymaware_name, completestgData);