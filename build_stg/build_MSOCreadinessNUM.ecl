...
sprayFile := Athlete360.files_spray.MSOCreadinessNUMfile;

// get the layout (processed layout)
stgLayout := Athlete360.Layouts.MSOCreadinessNUM_stg;

// do all preprocessing actions and get the cleaned data from spray
stgLayout extractdata (Athlete360.Layouts.MSOCreadinessNUM L):= transform
...
finalStageData := DEDUP(
        SORT(
            cleanedSprayFile + Athlete360.files_stg.MSOCreadinessNUM_stgfile,
            NAME, DATE, TIME, -wuid),
        NAME, DATE, TIME
    );
...
replaceMediansOnEmptycompletedatas := JOIN
    (
        ...
        TRANSFORM({RECORDOF(LEFT)},
									...
									self.DayNum := IF(left.DayNum <> 0, Left.DayNum, Right.DayNum);
...
EXPORT build_MSOCreadinessNUM := Athlete360.util.fn_promote_ds(Athlete360.util.constants.stg_prefix,  Athlete360.util.constants.MSOCreadinessNUM_name, replaceMediansOnEmptycompletedatas);